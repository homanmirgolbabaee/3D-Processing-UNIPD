...
void BasicSfM::solve()
{
  ...
  while( true )
  {
    ...
    std::cout << "Selected seed pair: (" << seed_pair_idx0 << ", " << seed_pair_idx1 << ") with " << max_corr << " correspondences." << std::endl;
...
bool BasicSfM::incrementalReconstruction( int seed_pair_idx0, int seed_pair_idx1 )
{
  ...
  // Check that the number of inliers for the Essential matrix is greater than the Homography matrix
  ...
  std::cout << "Intrinsics Matrix: " << intrinsics_matrix << std::endl;
  int inliers = cv::recoverPose(E, points0, points1, intrinsics_matrix, R, t, inlier_mask_E);
  // Check if the recovered transformation is mainly sideward motion
  ...
  for(int iter = 1; iter < num_cam_poses_ - 1; iter++ )
  {
      ...
      if( cam_pose_optim_iter_[cam_idx] > 0 )
      {
        for( auto const& co_iter : cam_observation_[cam_idx] )
        {
              ...
              cam_observation_[new_cam_pose_idx].find(pt_idx ) != cam_observation_[new_cam_pose_idx].end() )
          {
            ...
            cv::Mat R0, R1;
            cv::Rodrigues(cv::Mat(1, 3, CV_64F, cam0_data), R0);
            cv::Rodrigues(cv::Mat(1, 3, CV_64F, cam1_data), R1);
            cv::Mat P0 = intrinsics_matrix * (cv::Mat_<double>(3, 4) << R0.at<double>(0, 0), R0.at<double>(0, 1), R0.at<double>(0, 2), cam0_data[3],
                                        R0.at<double>(1, 0), R0.at<double>(1, 1), R0.at<double>(1, 2), cam0_data[4],
                                        R0.at<double>(2, 0), R0.at<double>(2, 1), R0.at<double>(2, 2), cam0_data[5]);
            


            cv::Mat P1 = intrinsics_matrix * (cv::Mat_<double>(3, 4) << R1.at<double>(0, 0), R1.at<double>(0, 1), R1.at<double>(0, 2), cam1_data[3],
                                        R1.at<double>(1, 0), R1.at<double>(1, 1), R1.at<double>(1, 2), cam1_data[4],
                                        R1.at<double>(2, 0), R1.at<double>(2, 1), R1.at<double>(2, 2), cam1_data[5]);

            std::vector<cv::Point2d> pts0 = { points0[co_iter.second] };
            std::vector<cv::Point2d> pts1 = { points1[cam_observation_[new_cam_pose_idx][pt_idx]] };
    ...
    //    return false;
        ...
        // int bad_reconstruction = 0;
        for (int i = 0; i < num_points_; ++i) {
            if (cam_pose_optim_iter_[i] > 0){ 
                const double *camera = cameraBlockPtr(i);
                if (std::isnan(camera[0]) || std::isnan(camera[1]) || std::isnan(camera[2]) ||
                    std::isnan(camera[3]) || std::isnan(camera[4]) || std::isnan(camera[5])) {
        ...
        if (divergence_detected) {
            std::cout << "Divergence detected. Restarting reconstruction." << std::endl;
            return false;
        }
    /////////////////////////////////////////////////////////////////////////////////////////
      ...
      
      {
        ...
        //////////////////////////////////////////////////////////////////////////////////
          ...
          ceres::CostFunction *cost_function =
              ReprojectionError::Create(observations_[2 * i_obs], observations_[2 * i_obs + 1]);

  
          problem.AddResidualBlock(cost_function,
                                    new ceres::CauchyLoss(2 * max_reproj_err_),
                                    parameters_.data() + camera_block_size_ * cam_pose_index_[i_obs],
                                    parameters_.data() + num_cam_poses_ * camera_block_size_ + point_block_size_ * point_index_[i_obs]);
          }        
