  ...
  for( int i = 0; i < images_names_.size(); i++  )
  {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
        ...
        for (auto &kp : keypoints) {
            colors.push_back(img.at<cv::Vec3b>(kp.pt));
        }
  ...
  for( int i = 0; i < images_names_.size() - 1; i++ )
  {
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      ...
      /////////////////////////////////////////////////////////////////////////////////////////
        ...
        // Perform geometric validation using Essential matrix and Homography matrix
        ...
        cv::Mat E = cv::findEssentialMat(points1, points2, new_intrinsics_matrix_, cv::RANSAC, 0.999, 1.0, inlier_mask_E);
        cv::Mat H = cv::findHomography(points1, points2, cv::RANSAC, 1.0, inlier_mask_H);

        // Check inliers
        int inliers_E = cv::countNonZero(inlier_mask_E);
        int inliers_H = cv::countNonZero(inlier_mask_H);

        if (inliers_E > inliers_H && inliers_E > 5) {
