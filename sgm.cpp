...
namespace sgm
{
  ...
  void SGM::compute_path_cost(int direction_y, int direction_x, int cur_y, int cur_x, int cur_path)
  {
    ...
    {
      // please fill me !
      // First pixel in the path has no predecessors
    ...
    {
      //Please fill me!
      // For other pixels in the path, aggregate the cost with the previous pixel
      int prev_x = cur_x - direction_x;
      int prev_y = cur_y - direction_y;
      
      // Find the minimum cost at the previous pixel for the smoothness term calculation
      unsigned long min_prev_cost = *min_element(path_cost_[cur_path][prev_y][prev_x].begin(), path_cost_[cur_path][prev_y][prev_x].end());

      for (int d = 0; d < disparity_range_; ++d) {
        // The cost without any penalty if the disparity does not change
        no_penalty_cost = path_cost_[cur_path][prev_y][prev_x][d];

        // The cost with a small penalty for a small disparity change
        small_penalty_cost = min_prev_cost + p1_;

        // The cost with a large penalty for larger disparity changes
        big_penalty_cost = min_prev_cost + p2_;

        // Compute the path cost at the current disparity
  ...
  void SGM::aggregation()
  {
    ...
    for(int cur_path = 0; cur_path < PATHS_PER_SCAN; ++cur_path)
    {
      ...
      // Depending on the path direction, we set different starting and ending points
      start_x = (dir_x > 0) ? 0 : (dir_x < 0 ? width_ - 1 : 0);
      end_x = (dir_x > 0) ? width_ : (dir_x < 0 ? -1 : width_);
      step_x = (dir_x == 0) ? 1 : dir_x;
      
      start_y = (dir_y > 0) ? 0 : (dir_y < 0 ? height_ - 1 : 0);
      end_y = (dir_y > 0) ? height_ : (dir_y < 0 ? -1 : height_);
      step_y = (dir_y == 0) ? 1 : dir_y;
